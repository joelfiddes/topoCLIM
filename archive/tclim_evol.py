...
def plot_hs(fsmfiles):
    ...
    with PdfPages(root + '/tclim_hs_plots.pdf') as pdf:
        for f in tqdm(fsmfiles):
            ...
            hs = df.iloc[:, col]
            try:
                title = f.split(grid)[1].split("_F.txt_11111.txt")[
                    0].split("fsm/output/")
...
Parallel(n_jobs=int(num_cores))(delayed(tclim_main)(tscale_file)
                                for tscale_file in tscale_files)
...
Parallel(n_jobs=int(num_cores))(delayed(met2fsm_parallel)(qfile)
                                for qfile in qfiles)
# cleanup _HOURLY.txt
...
Parallel(
    n_jobs=int(1))(
        delayed(fsm_sim)(
            meteofile,
            namelist,
            fsmexepath) for meteofile in meteofiles)
...
grid = 'g4'
sample = '5'
file = "/home/joel/sim/qmap/topoclim_test_hpc/" + grid + "/smeteoc" + \
    sample + "_1D/fsm/ICHEC-EC-EARTH_CLMcom-CCLM5-0-6_HIST_Q.txt"
q1 = pd.read_csv(file, parse_dates=True)
q1 = pd.read_csv(file, parse_dates=True)
...
file = "/home/joel/sim/qmap/topoclim_test_hpc/" + grid + "/smeteoc" + \
    sample + "_1D/fsm/CNRM-CERFACS-CNRM-CM5_CLMcom-CCLM5-0-6_HIST_Q_H.txt"
