...
def main(raw_dir, mylon, mylat, tz, nc_standard_clim, nc_standard_hist, cal_period, val_period, plot_period, path_inpt, outdir):  # create 1h wfj era5 obs data by
    ...
    data_obs_hourly = df_interpol.loc[:,
                                      ('TA', 'PINT', 'RH', 'ISWR', 'ILWR', 'VW')]
    ...
    for nc in ncfiles:
        ...
        try:
            #df = ds.to_dataframe()
            # , 'vas']]
            df2 = df[['tas', 'tasmin', 'tasmax',
                      'pr', 'hurs', 'rsds', 'rlds', 'ps']]
            # print 'ok'
            print(nc)
            print(df.columns)
    ...
    for nc in nc_complete:
        ...
        # sthis file has a standard calender and used as target in
        # interpolation (hist or clim periods)
        if 'historical' in nc:
        ...
        if datetimeindex.is_all_dates is False:
            ...
            # now we have to fill na in first and last timestep which results
            # from interpolation
        ...
        # if datetimeindex.is_all_dates is False: # this means we have 365 or
        # 360 day calender
        ...
        df = ds.sel(
            ...
            lat=latitude,
            method='nearest').to_dataframe()
        df2 = df.loc[:, ('tas', 'tasmin', 'tasmax', 'pr',
                         'hurs', 'rsds', 'rlds', 'ps')]
        # compute wind speed and wind direction
        df2.loc[:, 'ws'] = np.sqrt(df.uas**2 + df.vas**2)
        df2.loc[:, 'wd'] = (180 / np.pi) * np.arctan(df.uas / df.vas) + \
        ...
        if test_methods is True:
            ...
            station2.disaggregate_temperature(
                method='mean_course_mean', min_max_time='sun_loc_shift')
