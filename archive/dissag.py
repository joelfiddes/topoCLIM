...
# resample 3h OBS to 1h data (psum no longer valid) - can use 1h data
# directly here in future
...
ncfiles = glob.glob(
    '/home/joel/sim/qmap/test/pyout/aresult/' +
    "*_TS_ALL_ll.nc")
nc_complete = []
for nc in ncfiles:
...
# </p>
    ...
    try:
        #df = ds.to_dataframe()
        # , 'vas']]
        df2 = df[['tas', 'tasmin', 'tasmax', 'pr', 'hurs', 'rsds', 'rlds', 'ps']]
        # print 'ok'
        print (nc)
        print (df.columns)
...
for nc in nc_complete:
    ...
    if datetimeindex.is_all_dates is False:
        try:
            ...
            print(
                "Non standard time index 360 day cal, cant create datetime index, skipping")
    ...
    df2['wd'] = (180 / np.pi) * np.arctan(df.uas / df.vas) + \
        np.where(df.vas > 0, 180, np.where(df.uas > 0, 360, 0))
    ...
    if test_methods is True:
        ...
        for method in (
            ...
            station.disaggregate_temperature(
                method=method, min_max_time='sun_loc_shift')
        ...
        station2.disaggregate_temperature(
            method='mean_course_mean',
    ...
    station.data_disagg['DW'] = df4.wd.resample(
        'H').mean().interpolate(method='pad')

    # make air pressure equal each hour
    station.data_disagg['P'] = df4.ps.resample(
        'H').mean().interpolate(method='pad')
    ...
    if geotop_out is True:
        ...
        df_gtop.to_csv(
            path_or_buf='/home/joel/sim/qmap/geotop_sim/meteo0001.txt',
            na_rep=-999,
            float_format='%.3f')
    ...
    if fsm_out is True:
        ...
        df_fsm.to_csv(
            path_or_buf='/home/joel/sim/qmap/fsm/' +
            outname +
            '_HOURLY.txt',
            na_rep=-
            999,
...
for mymodel in range(0, len(models2)):
    ...
    if len([i for i, e in enumerate(models1)
            if e == models2[mymodel]]) >= len(pars):
        counter = counter + 1
        print(counter)
